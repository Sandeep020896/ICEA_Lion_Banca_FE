...
function fmtKES(n, compact=false) {
  return n == null ? "—" : compact ? fmtM(n) : `KES ${fmt(n)}`;
}
...
// ─── ANIMATED NUMBER ──────────────────────────────────────────────────────────
function AnimatedNumber({ target, duration=900, prefix="KES ", compact=false }) {
  ...
  const shown = compact ? fmtM(display) : `${prefix}${fmt(display)}`;
  ...
  return (
    <span style={{ position:"relative", display:"inline-block" }}
      ...
      <span style={{
        ...
        width:14, height:14, borderRadius:"50%", background:"#27AAE1",
      ...
      {show && (
        <div style={{
          ...
          transform:"translateX(-50%)", background:"#1a202c",
          color:"#fff", fontSize:11, lineHeight:1.5, padding:"8px 12px",
          borderRadius:7, width:220, zIndex:200, pointerEvents:"none",
...
// ─── LOGOS ─────────────────────────────────────────────────────────────────────
// TO REPLACE WITH REAL IMAGES: swap <IceaLionLogo/> with <img src="/icea_lion_logo.png" .../>
// File name expected in /public: icea_lion_logo.png
  ...
  return (
    <svg height={height} viewBox="0 0 160 48" fill="none" xmlns="http://www.w3.org/2000/svg">
      ...
      {[0,30,60,90,120,150,180,210,240,270,300,330].map((d,i)=>(
        <rect key={i} x="20.5" y="14" width="3" height="8" rx="1.5"
          fill="#B8962E" transform={`rotate(${d} 22 22)`} opacity="0.85"/>
      ))}
...
// TO REPLACE WITH REAL IMAGE: swap <CoherentLogo/> with <img src="/coherent_logo.png" .../>
// File name expected in /public: coherent_logo.png
function CoherentLogo({ height=22 }) {
  return (
    <svg height={height} viewBox="0 0 110 30" fill="none" xmlns="http://www.w3.org/2000/svg">
      {/* Coherent arc mark - two dark arcs + one purple arc */}
    ...
    <div style={{ display:"flex", flexDirection:"column", alignItems:"center", padding:"52px 0" }}>
      <svg width="80" height="80" viewBox="0 0 100 100" fill="none" xmlns="http://www.w3.org/2000/svg"
        ...
        {[0,30,60,90,120,150,180,210,240,270,300,330].map((deg,i)=>(
          <rect key={i} x="48" y="26" width="4" height="13" rx="2"
            fill="#B8962E" transform={`rotate(${deg} 50 46)`} opacity="0.8"/>
        ))}
...
function Field({ label, hint, error, disabled, children }) {
  return (
    <div style={{ marginBottom:16, opacity: disabled ? 0.5 : 1, transition:"opacity 0.2s" }}>
      ...
      {options.map(o => {
        ...
        return (
          <button key={o.value} onClick={()=>onChange(o.value)} style={{
            flex:1, padding:"9px 10px", border:"none", cursor: disabled?"not-allowed":"pointer",
...
// ─── STAT BOX ─────────────────────────────────────────────────────────────────
function StatBox({ label, value, sub, accent, tooltip }) {
  const inner = (
    <div style={{ padding:"14px 16px", borderRadius:9,
      ...
      <div style={{ fontSize:10, letterSpacing:"0.1em", textTransform:"uppercase",
        ...
        {tooltip
          ? <Tip text={tooltip}>{label}</Tip>
          : label}
      </div>
      <div style={{ fontSize:19, fontWeight:700,
        color: accent?"#fff":"#1a202c", fontFamily:"Georgia,serif" }}>{value}</div>
...
function PasswordScreen({ onUnlock }) {
  ...
  return (
    <div style={{ minHeight:"100vh", background:"linear-gradient(135deg,#001f3f,#003C7A)",
      display:"flex", alignItems:"center", justifyContent:"center",
      fontFamily:"'Inter','Helvetica Neue',sans-serif" }}>
      <div style={{ background:"#fff", borderRadius:16, padding:"48px 44px", width:380,
        boxShadow:"0 24px 80px rgba(0,0,0,0.3)", textAlign:"center" }}>
        <IceaLionLogo height={48}/>
        <div style={{ marginTop:24, marginBottom:6, fontSize:18, fontWeight:700,
          color:"#003C7A", fontFamily:"Georgia,serif" }}>
          Bancassurance Partner Portal
        </div>
        <div style={{ fontSize:12, color:"#718096", marginBottom:28 }}>
        ...
        <div style={{ position:"relative", marginBottom:16 }}>
          <input
            ...
            style={{ width:"100%", boxSizing:"border-box", padding:"12px 44px 12px 16px",
              fontSize:15, border:`1.5px solid ${err?"#c0392b":"#e2e8f0"}`,
          ...
          <button onClick={()=>setShow(s=>!s)} style={{ position:"absolute", right:12,
            ...
            cursor:"pointer", color:"#718096", fontSize:11 }}>
            {show?"Hide":"Show"}
          </button>
        </div>
        {err && <div style={{ fontSize:11, color:"#c0392b", marginBottom:10 }}>
        ...
        <div style={{ marginTop:20, fontSize:10, color:"#a0aec0" }}>
...
function TimeoutModal({ onStay, onLogout }) {
  return (
    <div style={{ position:"fixed", inset:0, zIndex:2000,
      background:"rgba(0,0,0,0.6)", display:"flex",
      alignItems:"center", justifyContent:"center" }}>
      <div style={{ background:"#fff", borderRadius:14, padding:"36px 40px",
        width:360, textAlign:"center", boxShadow:"0 24px 80px rgba(0,0,0,0.3)" }}>
        ...
        <div style={{ fontSize:13, color:"#718096", marginBottom:24, lineHeight:1.6 }}>
          You have been inactive for 30 minutes. Do you want to continue your session?
        </div>
...
// ─── EMAIL MODAL ──────────────────────────────────────────────────────────────
function EmailModal({ open, onClose, result, inputs, quoteRef, pdfUrl }) {
  ...
  const subject   = `Quotation ${quoteRef} – Summit Endowment Plan – ICEA LION Life Assurance`;

  const body = [
    `Dear ${recipName || "Sir/Madam"},`,
    "",
    ...
    "",
    pdfUrl ? `View your quotation PDF here: ${pdfUrl}` : `Please find the PDF quotation attached to this email.`,
    "",
    `To proceed, please complete the proposal form. A medical report may be required in some cases.`,
    "",
    `Warm regards,`,
    senderName || "[Your Name]",
    `ICEA LION Life Assurance – Bancassurance Partner`,
  ].join("\n");
      ...
      <div style={{ background:"#fff", borderRadius:14, width:520,
        ...
        <div style={{ background:"#003C7A", padding:"16px 24px",
          ...
          <button onClick={onClose} style={{ background:"none", border:"none",
            color:"rgba(255,255,255,0.6)", cursor:"pointer", fontSize:20, lineHeight:1 }}>×</button>
        </div>
        ...
        <div style={{ padding:"22px 24px" }}>
          <div style={{ background:"#f7fafc", border:"1px solid #e2e8f0",
            ...
            <strong style={{ color:"#003C7A" }}>Note:</strong> This opens your email client with the quotation
            details pre-filled. Attach the downloaded PDF separately.
            Full email integration (auto-attach PDF) requires a backend email service — speak to your
            Coherent account manager to enable this.
          </div>
          ...
          <div style={{ marginBottom:14 }}>
            ...
            <input type="text" value={recipName}
              onChange={e=>setRecipName(e.target.value)}
              placeholder={inputs.clientName || "Client name"}
              style={{ width:"100%", boxSizing:"border-box", padding:"9px 12px",
                border:"1.5px solid #e2e8f0", borderRadius:7, fontSize:13,
                fontFamily:"inherit", outline:"none" }}/>
          </div>
          ...
          <div style={{ marginBottom:14 }}>
            ...
            <input type="text" value={senderName}
              ...
              placeholder="e.g. John Kamau – Equity Bank Westlands"
              style={{ width:"100%", boxSizing:"border-box", padding:"9px 12px",
                border:"1.5px solid #e2e8f0", borderRadius:7, fontSize:13,
                fontFamily:"inherit", outline:"none" }}/>
          </div>
          ...
          <div style={{ background:"#f0f7ff", border:"1px solid #c3d9ef",
            ...
            <div style={{ fontSize:10, letterSpacing:"0.1em", textTransform:"uppercase",
              color:"#003C7A", fontWeight:700, marginBottom:6 }}>Email will include</div>
...
function PdfPreviewModal({ open, onClose, pdfUrl, quoteRef, clientName }) {
  ...
  // Blob-fetch download so it actually saves to disk instead of opening a tab
    ...
    <div style={{ position:"fixed", inset:0, zIndex:1200,
      ...
      <div style={{ background:"#fff", borderRadius:14, width:"100%", maxWidth:860,
        ...
        <div style={{ background:"#525659", borderRadius:"0 0 14px 14px",
          overflow:"hidden", position:"relative", minHeight:700 }}>

          {!embedFailed ? (
            /* <object> is more permissive than iframe for PDF embedding */
            <object
              data={pdfUrl}
              ...
              {/* If object also fails, this inner content shows */}
              <embed
                src={pdfUrl}
                type="application/pdf"
                width="100%"
          ...
          {/* Always show fallback message underneath if needed */}
          {embedFailed && (
            <div style={{ display:"flex", flexDirection:"column", alignItems:"center",
              justifyContent:"center", height:500, gap:20, padding:"40px",
              textAlign:"center" }}>
              ...
              <div>
                ...
                <div style={{ fontSize:12, color:"rgba(255,255,255,0.55)", lineHeight:1.7,
                  ...
                  Your browser security settings are blocking the inline PDF viewer.
                  Use <strong style={{ color:"#fff" }}>Download PDF</strong> above to
                  save the file, or open it directly below.
                </div>
              </div>
...
export default function ICEALionBancaPortal() {
  ...
  function resetIdle() {
    ...
    if (unlocked) {
      idleTimer.current = setTimeout(()=>setShowTimeout(true), IDLE_TIMEOUT_MS);
    }
  }
  useEffect(()=>{
    if (!unlocked) return;
    ...
    (async()=>{
      ...
      try {
        ...
        if (v) {
          ...
          // ClientName is now a confirmed Xinput (Coherent!C8) - pre-fill but leave blank
          // so bank teller always enters the actual client name. Default from model is test data.
        } else { setValError("Could not load form rules."); }
        // Version comes back in response_meta of the validation call
  ...
  const validate = useCallback(()=>{
    ...
    return errs;
  },[age,sa,premium,calcby,validation,clientName]);
  ...
  async function handleGetQuote() {
    ...
    // Build ref: YYYYMMDD-ClientName-counter
    const d = new Date();
    const datePart = `${d.getFullYear()}${String(d.getMonth()+1).padStart(2,"0")}${String(d.getDate()).padStart(2,"0")}`;
    const namePart = clientName.trim().replace(/\s+/g,"") || "Client";
    ...
    const ref = `${datePart}-${namePart}-${String(counter).padStart(4,"0")}`;
    setQuoteRef(ref);
    setQuoteDate(new Date());

    setLoading(true); setApiError(null); setResult(null);
    setPdfUrl(null); setAcceptState("idle"); setFinalLoading(false);

    try {
      const data = await callSpark(`Get Quote - ${ref}`, false);
  ...
  async function handleAccept() {
    ...
    const purposeWithName = `Final Quotation - ${clientName.trim() || "Client"}`;
    try {
      const data = await callSpark(purposeWithName, true);
      if (data?.response_meta?.version) setModelVersion(data.response_meta.version);
      // Extract PDF URL from QUOTATION output
      const quotationOut = data?.response_data?.outputs?.QUOTATION;
      if (quotationOut?.PDFUrl) {
        setPdfUrl(quotationOut.PDFUrl);
        // PDF ready - user clicks "View PDF" to open the preview modal
      }
    } catch { /* non-blocking */ }
    finally {
  ...
  function handleClear() {
    ...
    setPdfUrl(null); setQuoteRef(""); setQuoteDate(null);
    const v = validation;
    setAge(v?.Age?.default_value ? String(v.Age.default_value) : "");
    setGender(v?.Gender?.default_value || "M");
    setSa(v?.SA?.default_value ? String(v.SA.default_value) : "");
    setPremium(v?.Premium?.default_value ? String(v.Premium.default_value) : "");
  ...
  // Always display monthly premium (model outputs annual, divide by 12)
  ...
  return (
    <div style={{ minHeight:"100vh", background:"#f0f4f8",
      ...
      {showTimeout && (
        <TimeoutModal
          onStay={()=>{ setShowTimeout(false); resetIdle(); }}
          onLogout={()=>{ setShowTimeout(false); setUnlocked(false); handleClear(); }}
        />
      )}
      ...
      <EmailModal
        ...
        quoteRef={quoteRef} pdfUrl={pdfUrl}
      />

      {/* ══ PDF PREVIEW MODAL ══ */}
      <PdfPreviewModal
        open={pdfPreviewOpen} onClose={()=>setPdfPreviewOpen(false)}
        pdfUrl={pdfUrl} quoteRef={quoteRef} clientName={clientName}
      />

      {/* ══ HEADER ══ */}
      <header style={{ background:"#fff", borderBottom:"1px solid #e2e8f0",
        padding:"0 36px", height:66, display:"grid",
        ...
        {/* Left: ICEA Lion logo
            → Replace <IceaLionLogo/> with <img src="/icea_lion_logo.png" alt="ICEA LION" style={{height:40}}/> */}
        <div style={{ display:"flex", alignItems:"center" }}>
          <IceaLionLogo height={38}/>
        </div>

        {/* Centre: Engine Live + KES toggle + Portal badge */}
        <div style={{ display:"flex", alignItems:"center", gap:14, justifyContent:"center" }}>
          ...
          <button onClick={()=>setCompact(c=>!c)} style={{
            ...
            {compact ? "KES Full" : "KES M"}
          </button>
          <div style={{ background:"#eef4fb", border:"1px solid #c3d9ef",
            borderRadius:6, padding:"5px 12px", fontSize:11, fontWeight:700,
            color:"#003C7A", letterSpacing:"0.07em", textTransform:"uppercase" }}>
            Bancassurance Portal
          </div>
        </div>

        {/* Right: Powered by Coherent
            → Replace <CoherentLogo/> with <img src="/coherent_logo.png" alt="Coherent" style={{height:22}}/> */}
      ...
      {/* ══ TITLE BAND ══ */}
      <div style={{ background:"#003C7A", padding:"28px 36px 20px" }}>
        <div style={{ maxWidth:1120, margin:"0 auto" }}>
          {/* Top row: empty left | centre title | ref right */}
          <div style={{ display:"grid", gridTemplateColumns:"1fr auto 1fr",
            ...
            <div style={{ textAlign:"center" }}>
              ...
              <div style={{ fontSize:11, letterSpacing:"0.18em", textTransform:"uppercase",
                color:"#27AAE1", marginTop:4, fontWeight:600 }}>
                Quotation Generator
              </div>
            </div>
              ...
              {quoteRef && (
                <>
                  <div style={{ fontSize:9, color:"rgba(255,255,255,0.4)",
                    letterSpacing:"0.08em", marginBottom:2 }}>QUOTATION REF</div>
                  <div style={{ fontSize:12, fontWeight:700, color:"rgba(255,255,255,0.75)",
                    fontFamily:"monospace" }}>{quoteRef}</div>
                </>
              )}
            </div>
          </div>
          {/* Bottom row: version pill bottom-left */}
          {modelVersion && (
          ...
          <div style={{ padding:"20px 22px" }}>
            ...
            <Field label="Client Name" error={fieldErrors.clientName}>
              <input type="text" style={inp("clientName")} value={clientName}
                ...
                placeholder="e.g. Amara Osei" disabled={loading}/>
            </Field>
            ...
            <Field label="I want to quote by"
              hint={calcby==="Sum Assured"
                ...
                :"Enter the premium the client can afford — the coverage will be calculated."}>
              <Toggle value={calcby}
            ...
            {calcby==="Sum Assured" ? (
              <Field label="Sum Assured (KES)" error={fieldErrors.sa}>
                <input type="number" style={inp("sa")} value={sa} disabled={loading}
              ...
              <Field label="Annual Premium (KES)" error={fieldErrors.premium}>
                <input type="number" style={inp("premium")} value={premium} disabled={loading}
            ...
            <Field label={`Age at Entry`} error={fieldErrors.age}
              ...
              <input type="number" style={inp("age")} value={age} min={ageMin} max={ageMax}
                disabled={loading}
            ...
            <Field label="Gender" disabled={loading}>
              <Toggle value={gender}
          ...
          {!result && !loading && !apiError && (
            <div style={{ background:"#fff", borderRadius:13, border:"1px dashed #cbd5e0",
              ...
              <div style={{ fontSize:12, color:"#a0aec0", lineHeight:1.6,
                ...
                Complete the form and click <strong style={{ color:"#003C7A" }}>Get Quote</strong>.
              </div>
            </div>
          )}
          ...
          {result && !loading && (
            <div style={{ animation:"slideIn 0.3s ease-out" }}>
              ...
              <div style={{ background:"#fff", borderRadius:13, border:"1px solid #e2e8f0",
                ...
                {/* Navy header */}
                <div style={{ background:"#003C7A", padding:"15px 22px",
                  display:"flex", justifyContent:"space-between", alignItems:"center" }}>
                  <div>
                    ...
                    <div style={{ fontSize:15, fontWeight:700, color:"#fff",
                      fontFamily:"Georgia,serif" }}>Summit Endowment Plan</div>
                    {clientName && (
                ...
                {/* Policy strip */}
                <div style={{ padding:"9px 22px", background:"#f7fafc",
                  borderBottom:"1px solid #e8eef4", display:"flex", gap:22,
                ...
                <div style={{ padding:"16px 22px" }}>
                  ...
                  {validUntil && (
                    <div style={{ background:"#f0f7ff", border:"1px solid #c3d9ef",
                      ...
                      <svg width="13" height="13" viewBox="0 0 24 24" fill="none"
                        stroke="#003C7A" strokeWidth="2" strokeLinecap="round">
                        <circle cx="12" cy="12" r="10"/>
                        <polyline points="12 6 12 12 16 14"/>
                      </svg>
                      <span style={{ fontSize:11, color:"#2c5282" }}>
                        This quotation is valid for <strong>90 days</strong> — expires <strong>{validUntil}</strong>
                      </span>
                    </div>
                  )}
                  ...
                  <div style={{ display:"grid", gridTemplateColumns:"1fr 1fr 1fr",
                    ...
                    <StatBox label="Maturity Estimate"
                      ...
                      tooltip="The estimated value you receive at the end of the policy term. Includes your Sum Assured plus accumulated bonuses. Bonuses may vary year on year based on market performance."/>
                    <StatBox label="Bonus Estimate"
                      value={<AnimatedNumber target={bonusEst} compact={compact}/>}
                      sub="Simple interest on SA"
                      tooltip="The projected bonus applied to your Sum Assured over the policy term. This is an estimate only — actual bonuses are declared annually and depend on investment returns."/>
                  </div>
                  ...
                  <div style={{ display:"grid", gridTemplateColumns:"1fr 1fr",
                    ...
                    <StatBox label="Maturity Growth"
                      ...
                      sub="vs total premiums paid" accent/>
                  </div>
                  ...
                  <div style={{ background:"#fffbeb", border:"1px solid #f6e05e",
                    ...
                    <div style={{ fontSize:11, fontWeight:700, color:"#744210", marginBottom:3 }}>
                      Year-by-year maturity projection
                    </div>
                    <div style={{ fontSize:11, color:"#92400e", lineHeight:1.5 }}>
                      End-of-term estimate: KES {fmt(maturityEst)}.
                  ...
                  <div style={{ background:"#f7fafc", borderRadius:7, padding:"12px 14px" }}>
                    <div style={{ fontSize:10, letterSpacing:"0.1em", textTransform:"uppercase",
                      color:"#718096", fontWeight:700, marginBottom:7 }}>Notes</div>
                    {[
              ...
              {acceptState==="pending" && (
                <div style={{ background:"#fff", borderRadius:13, border:"1.5px solid #003C7A",
                  ...
                  <div style={{ background:"linear-gradient(135deg,#003C7A,#0057a8)",
                    padding:"18px 24px" }}>
              ...
              {acceptState==="accepted" && (
                <div style={{ background:"#f0faf4", borderRadius:13,
                  ...
                  <div style={{ display:"flex", flexDirection:"column", alignItems:"center", gap:10 }}>
                    ...
                    <div>
                      <div style={{ fontSize:14, fontWeight:700, color:"#276749" }}>
                        Quote Accepted — {clientName}
                      </div>
                      <div style={{ fontSize:12, color:"#48bb78", marginTop:3 }}>
                        Final quotation recorded.
                        {pdfUrl ? " Quotation PDF ready." : " PDF is being generated."}
                      </div>
                    </div>
                  </div>
                  {/* Buttons centred */}
                  <div style={{ display:"flex", gap:10, justifyContent:"center", flexWrap:"wrap" }}>
                    {pdfUrl ? (
                      <button onClick={()=>setPdfPreviewOpen(true)}
                        style={{ padding:"9px 20px", background:"#003C7A",
                      ...
                      <button disabled style={{ padding:"9px 20px", background:"#e2e8f0",
                        ...
                        display:"flex", alignItems:"center", gap:7 }}>
                        <Spinner size={12} color="#a0aec0"/> Generating PDF…
                      </button>
                    )}
                    ...
                    <button onClick={handleDecline}
                      style={{ padding:"9px 20px", background:"#fff",
      ...
      {/* ══ FOOTER ══ */}
      <footer style={{ maxWidth:1120, margin:"8px auto 0", padding:"14px 36px",
        borderTop:"1px solid #e2e8f0", display:"flex",
        justifyContent:"space-between", fontSize:11, color:"#a0aec0" }}>
